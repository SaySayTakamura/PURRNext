...
{
    //Client used to fetch content from E6's API
    private IE621Client? iClient = null;
    //Tags used for searching with the API
    private string iTags = "";
    //The amount of pages to be searched
    //If the value is equal to 0 no pagination will be done and will retrieve the max of posts allowed
    private int iPages = 0;
    //The Output Directory where THIS search will save the files.
    private string OutputDir = "";
    //An INSTANCE of the Logger to report issues and save data to Logging Files
    private Logger? LoggerInstance = null;
    //Reference to a Config instance to fetch and check User Options within this process
    private Configuration? Configuration = null;
    //An Entry Form for Logging
    //Used mostly to document the progress as a formulary.
    private EntryForm ef;
    //The blacklist string
    //Used here to exclude the blacklist from the directories name.
    private string BlacklistString = "";
    //Database Storage Context
    //Used to save posts data;
    private DatabaseContext dbContext;
    private bool dbImplementationTest = true;
    ...
    public void Start()
    {
        ...
        if (iPages == 0)
        {
            ...
            {
                Console.WriteLine($"An error has occurred when fetching posts\n{e}");
            }
        }
        else
        {
                ...
                for (int p = 0; p < iPages; p++)
                {
                    ...
                    if (ps.IsCompleted)
                    {
                        ...
                        if (result.Count != 0)
                        {
                            ...
                            Console.WriteLine($"Total fetched posts for this page - {PostGallery.Count}");
                        }
                        else
                        {
                            Console.WriteLine("No posts found for this page");
                            //This shows the workload size using the proper sufix convention depending on the size
                            Console.WriteLine($"Current Workload Size is {BytesToString(WorkloadSize)}!");
                            //Console.WriteLine("Quitting to avoid Cluttering Storage.");
                            //Console.ReadLine();
                            //Environment.Exit(0);


                            /*
                                This go-to is not necessarily needed
                                But in case the user inputs a number of pages that exceeds
                                the number of pages available at the origin (e621)
                                The system would keep iterating until it reached the provided number of pages.
                                ...
                                So... Let's keep at it.
                            */
                            goto F_PROCESS;
        ...
        Console.WriteLine("This is being called due code hirearchy\nWhich means that no 'goto' directive was used and this is happening because the last code has ended.");
F_PROCESS:
        try
        {
            if(dbImplementationTest == false)
            {
                //Pools dictionary
                ...
                */
                ...
                if (Pools.Count > 0)
                {
                    ...
                    if (Duplicates.Count > 0)
                    {
                        ...
                        {
                            //Assign a duplicate to a variable
            ...
            else
            {
                ...
                var FlashDir = $"{OutputDir}Flash/";
                var VideoDir = $"{OutputDir}Videos/";
                //Separation list for the above types
                List<Post> Flashes = new List<Post>(); //Goes in the /Flash/ Folder
                List<Post> Videos = new List<Post>(); // Goes in the /Videos/ Folder
                List<Post> Normals = new List<Post>();


                Console.WriteLine("Database checkup starting!");
                for(int p = 0; p < PostGallery.Count; p++)
                {
                    var post = PostGallery[p];

                    var exists = dbContext.PostExists(post.Id);
                    exists.Wait();

                    if(exists.Result == true)
                    {
                        //Post exists on the database and so should its file on the file system.
                        //Remove from the list
                        //PostGallery.RemoveAt(p);
                        Console.WriteLine($"Post - {post.Id} already exists in the database, nothing to do.");
                    }
                    else
                    {
                        Console.WriteLine($"Post - {post.Id} - Does not exist in the database, preparing");
                        var postPath = "";
                        if(post.File.FileExtension == "swf")
                        {
                            Console.WriteLine("Post content is FLASH");
                            //Checks for videos and flashes goes here
                            if(Configuration.FlashOnFolders == true)
                            {
                                Console.WriteLine("Flash separation is enabled");
                                postPath = FlashDir;
                                Flashes.Add(post);
                            }
                            else
                            {
                                Console.WriteLine("Flash separation is disabled");
                                postPath = OutputDir;
                                Normals.Add(post);
                            }
                        }
                        else if (post.File.FileExtension == "webm" || post.File.FileExtension == "mp4")
                        {
                            Console.WriteLine("Post content is VIDEO");
                            if(Configuration.VideoOnFolders == true)
                            {
                                Console.WriteLine("Video separation is enabled");
                                postPath = VideoDir;
                                Videos.Add(post);
                                //PostGallery.RemoveAt(p);
                            }
                            ...
                            {
                                Console.WriteLine("Video separation is disabled");
                                postPath = OutputDir;
                                Normals.Add(post);
                            }
                        }
                        ...
                        {
                            Console.WriteLine("Post content is IMAGE/GIF");
                            postPath = OutputDir;
                            Normals.Add(post);
                        }
                        Console.WriteLine("Inserting post in the database");
                        var insert = dbContext.AddPost(post, postPath);
                        insert.Wait();
                        if(insert.Result == true)
                        {
                            Console.WriteLine("Post successfuly added to the database");
                        }
                    }
                }
